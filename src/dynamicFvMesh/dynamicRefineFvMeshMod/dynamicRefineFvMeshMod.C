...
#include "dynamicRefineFvMeshMod.H"
...
namespace Foam
{
    defineTypeNameAndDebug(dynamicRefineFvMeshMod, 0);
    addToRunTimeSelectionTable(dynamicFvMesh, dynamicRefineFvMeshMod, IOobject);
    addToRunTimeSelectionTable(dynamicFvMesh, dynamicRefineFvMeshMod, doInit);
}

// * * * * * * * * * * * * Protected Member Functions  * * * * * * * * * * * //
void Foam::dynamicRefineFvMeshMod::updateDynamicRefineFvMeshModFaceMap
(
    const labelList& newFaceMap
)
{
    const labelList oldFaceMap = faceMap0_;
    
    faceMap0_.setSize(newFaceMap.size(), -1);
    
    // The final map needs to have the size of the latest (unrefined) list of faces
    forAll(newFaceMap, facei)
    {
    	faceMap0_[facei] = oldFaceMap[newFaceMap[facei]];
    }
}

void Foam::dynamicRefineFvMeshMod::calculateProtectedCells
(
...
void Foam::dynamicRefineFvMeshMod::readDict()
{
...
void Foam::dynamicRefineFvMeshMod::mapFields(const mapPolyMesh& mpm)
{
...
Foam::dynamicRefineFvMeshMod::refine
(
...
Foam::autoPtr<Foam::mapPolyMesh>
Foam::dynamicRefineFvMeshMod::unrefine
(
...
Foam::scalarField
Foam::dynamicRefineFvMeshMod::maxPointField(const scalarField& pFld) const
{
...
Foam::scalarField
Foam::dynamicRefineFvMeshMod::maxCellField(const volScalarField& vFld) const
{
...
Foam::scalarField
Foam::dynamicRefineFvMeshMod::cellToPoint(const scalarField& vFld) const
{
...
Foam::scalarField Foam::dynamicRefineFvMeshMod::error
(
...
void Foam::dynamicRefineFvMeshMod::selectRefineCandidates
(
...
Foam::labelList Foam::dynamicRefineFvMeshMod::selectRefineCells
(
...
Foam::labelList Foam::dynamicRefineFvMeshMod::selectUnrefinePoints
(
...
void Foam::dynamicRefineFvMeshMod::extendMarkedCells
(
    bitSet& markedCell
) const
{
...
void Foam::dynamicRefineFvMeshMod::checkEightAnchorPoints
(
...
Foam::dynamicRefineFvMeshMod::dynamicRefineFvMeshMod
(
    const IOobject& io,
    const bool doInit
)
...
bool Foam::dynamicRefineFvMeshMod::init(const bool doInit)
{
    if (doInit)
    {
...
bool Foam::dynamicRefineFvMeshMod::update()
{
    ...
    if (time().timeIndex() > 0 && time().timeIndex() % refineInterval == 0)
    {
...

        {
            ...
            if (nSplitPoints > 0)
            {
                ...
                autoPtr<mapPolyMesh> map = unrefine(pointsToUnrefine);
                
                // If the mesh is previously refined, update the map, otherwise just overwrite map.
                if (refinedMesh)
                {
                   updateDynamicRefineFvMeshModFaceMap(map().faceMap());
                }
                else
                {
                   faceMap0_ = map().faceMap();
...
bool Foam::dynamicRefineFvMeshMod::writeObject
(
