    ...
    // Part 2 - Load Balancing
    {
        ...
        if ( Pstream::parRun() && hasChanged )
        {
            ...
            if( maxImbalance > allowableImbalance && enableBalancing)
            {
                ...
                /*Info<< "Update the faceMap ..." << endl;
                labelList faceMap = dynamicMultiDimRefineFvMesh::dynamicMultiDimRefineFvMeshFaceMap();
                map().distributeFaceData(faceMap);
                //map().distributeFaceData(faceMap0_);
                dynamicMultiDimRefineFvMesh::updateDynamicMultiDimRefineFaceMap(faceMap);*/
