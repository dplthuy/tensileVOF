...
void Foam::dynamicMultiDimRefineFvMesh::updateDynamicMultiDimRefineFaceMap
(
...
)
{
    ...
    // The final map needs to have the size of the latest list of faces
    ...
    // Correct the flux for modified faces.
    {
        ...
        forAllIters(fluxes, iter)
        {
            ...
            forAllConstIters(faceToSplitPoint, iter)
            {
                ...
                const label oldPointi = iter();//iter.object();
    ...
    if (time().timeIndex() > 0 && time().timeIndex() % refineInterval == 0)
    {
...

        {
            ...
            if (nSplitElems > 0)
            {
                ...
                autoPtr<mapPolyMesh> map = unrefine(elemsToUnrefine);
                
                // If the mesh is previously refined, update the map, otherwise just overwrite map
		if (refinedMesh)
		{
		   updateDynamicMultiDimRefineFaceMap(map().faceMap());
		}
		else
		{
		   faceMap0_ = map().faceMap();
