...
namespace Foam
{
    defineTypeNameAndDebug(modRegionSplit, 0);
}
...
void Foam::modRegionSplit::checkBoundaryFaceSync
(
...
void Foam::modRegionSplit::updateFacePair
(
    const label face0,
    const label face1,
...
void Foam::modRegionSplit::fillSeedMask
(
...
Foam::label Foam::modRegionSplit::localRegionSplit
(
    const UList<labelPair>& explicitConnections,
...
) const
{
    ...
    DebugInfo << "modRegionSplit = " << double(timing.elapsed()) << "s\n";

    return nLocalRegions;
}

void Foam::modRegionSplit::cellRegionLabels
(
    const polyMesh& mesh,
    const labelList& cellRegion
)
{
    regionLocalCellLabels_.clear();
    regionLocalCellLabels_.setSize(nRegions());

    forAll(cellRegion, localCelli)
    {
        label regioni = cellRegion[localCelli];   
        regionLocalCellLabels_[regioni].append(localCelli);
    }
    
}

void Foam::modRegionSplit::cellRegionLabelsToGlobal()
{
    globalIndex globalCells(mesh_.nCells());
    
    regionGlobalCellLabels_.clear();
    regionGlobalCellLabels_.setSize(nRegions());

    forAll(regionLocalCellLabels_, regioni)
    {
        forAll(regionLocalCellLabels_[regioni], labeli)
        {
            label localCelli = regionLocalCellLabels_[regioni][labeli];
            label globalCelli = globalCells.toGlobal(localCelli);
            regionGlobalCellLabels_[regioni].append(globalCelli);
        }

        Pstream::combineAllGather(regionGlobalCellLabels_[regioni], ListOps::appendEqOp<label>());
    }
}

// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

Foam::modRegionSplit::modRegionSplit
(
    const polyMesh& mesh,
    const boolList& blockedFace,
...
:
    MeshObject<polyMesh, Foam::TopologicalMeshObject, modRegionSplit>(mesh),
    ...
    globalNumbering_()
{
    ...
    {
        globalNumbering_ =
            reduceRegionsImpl(numLocalRegions, blockedFace, cellRegion);
    }
    else
    {
    ...
    cellRegionLabels(mesh, cellRegion);
...
Foam::globalIndex
Foam::modRegionSplit::reduceRegionsImpl
(
    const label numLocalRegions,
    const boolList& blockedFace,
    labelList& cellRegion
) const
{
                ...
                forAll(faceCells, patchFacei)
                {
                    ...
                    if (!blockedFace.test(meshFacei))
                    {
                        ...
                        if (recv == UNASSIGNED)
                        {
                            if (emitWarning)
                            {
                                Pout<<"Warning in modRegionSplit:"
    ...
    DebugInfo
        <<"modRegionSplit::reduceRegions = " << double(timing.elapsed()) << "s\n";
